...
export const VendorRecentOrders: React.FC<VendorRecentOrdersProps> = ({
  ...
  return <Card className="dark-glass-effect border-slate-700">
      <CardHeader>
        <CardTitle className="text-xl font-semibold text-white">Recent Orders</CardTitle>
      </CardHeader>
      <CardContent>
        {orders.length > 0 ? <div className="space-y-4">
            {orders.slice(0, 5).map(order => <div key={order.id} className="flex items-center justify-between p-4 border border-slate-700 rounded-lg hover:bg-slate-800/50 transition-colors">
                <div>
                  <p className="font-medium text-slate-100">Order #{String(order.id).substring(0, 8)}</p>
          ...
          </div> : <p className="text-slate-400 text-center py-8">No orders yet</p>}
      </CardContent>
    </Card>;
};