...
const HeroSection: React.FC = () => {
  ...
  const features = [
    { icon: Leaf, text: "Fresh & Local", color: "green" },
    { icon: Heart, text: "Fair Trade", color: "red" },
    { icon: Truck, text: "Direct Delivery", color: "blue" },
    { icon: Users, text: "Community First", color: "purple" }
  ];
            ...
            <div className="flex flex-wrap justify-center gap-6 pt-16">
              {features.map((feature, index) => {
                ...
                return (
                  <div 
                    key={index} 
                    className={`group flex items-center gap-3 px-6 py-4 bg-gradient-to-r ${colorClasses[feature.color as keyof typeof colorClasses]} backdrop-blur-sm border rounded-2xl hover:scale-110 transition-all duration-500 cursor-pointer shadow-lg hover:shadow-2xl`}
                    style={{
