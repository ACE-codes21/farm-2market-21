...
import { Search, ShoppingCart, BarChart4, CreditCard, Users, Settings } from 'lucide-react';
...
const features = [{
  ...
  desc: 'Browse curated products & produce with vibrant cards and AI-powered recommendations. Add to cart in one click.',
  preview: <div className="feature-preview bg-black/50 border-orange-400/30 neon-box text-orange-300 rounded-xl">
  ...
  title: 'AI Analytics',
  desc: 'Visual dashboards show live sales & business insights. Instantly spot trends using animated, glowing charts.',
  preview: <div className="feature-preview bg-black/50 border-cyan-400/30 neon-box text-cyan-300 rounded-xl">
  ...
  desc: 'Get instant access to loans, government schemes, and repayment insights—all managed in a stunning finance hub.',
  preview: <div className="feature-preview bg-black/50 border-yellow-300/30 neon-box text-yellow-200 rounded-xl">
  ...
  desc: 'Secure chat, direct messaging, and local forums—building real relationships between buyers, vendors, and neighborhoods.',
  preview: <div className="feature-preview bg-black/50 border-pink-400/30 neon-box text-pink-300 rounded-xl">
  ...
  desc: 'Powerful, easy-to-use dashboards help vendors manage stock, feedback, finances & deliveries—automated and user-friendly.',
  preview: <div className="feature-preview bg-black/50 border-blue-300/30 neon-box text-blue-300 rounded-xl">
...
export const DynamicFeaturesSection: React.FC = () => {
  ...
  return <section ref={sectionRef} id="dynamic-features" className="relative px-2 md:px-0 py-32 overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto z-20">
        ...
        <div className="flex flex-col gap-28">
          {features.map((f, i) => {
          const isEven = i % 2 === 0;
          return <motion.div key={f.title} initial={isEven ? 'offscreenLeft' : 'offscreenRight'} animate={isInView ? 'onscreen' : isEven ? 'offscreenLeft' : 'offscreenRight'} variants={animationVariants} transition={{
            duration: 0.76,
            delay: 0.14 * i,
            type: 'spring',
            damping: 20,
            stiffness: 180
          }} className={`relative flex flex-col-reverse md:flex-row items-center gap-10 md:gap-20 group ${!isEven ? 'md:flex-row-reverse' : ''}`}>
                {/* Text & Icon */}
                <div className={`flex-shrink-0 w-full md:w-2/5 flex flex-col items-center md:items-start text-center md:text-left ${!isEven ? 'md:items-end md:text-right' : ''}`}>
                  {/* Icon with glowing pulse */}
                  <div className={`mb-7 flex items-center justify-center md:justify-start neon-icon-glow transition-transform group-hover:scale-110`}>
